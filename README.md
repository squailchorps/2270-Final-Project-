...
-	All parents can have at max 4 children. 
-	If the node is internal, it will always point to its child. 
-	If the node is a child, it will not point anywhere. 

The specific type of quad tree I have made are regional quadtrees, which are the division of space equally into 4 parts and its sub-quadrants.
...
Last pre check is that we check to see if there is a node with the same positional data as the one we want to insert. If that’s the case, we will do nothing and return. 
These above cases help get us out of our recursion loops below. 
...
Otherwise, it means it MUST have a connection somewhere else, so it must have a connector that looks like this ├── and adds | to it to extend it down. 
